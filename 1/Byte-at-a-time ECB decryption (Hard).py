...
randomPrefix = b"".join([Random.new().read(1) for i in range(int.from_bytes(Random.new().read(1), byteorder='little'))])
...
def pad(your_string, msg):
    ...
    paddedMsg = randomPrefix + your_string + msg
...
def ecb_decrypt(block_size):
    tmp_cipher = encryption_oracle(b"A"*block_size*3)
    Blocks = [tmp_cipher[i*block_size:][:block_size] for i in range(len(tmp_cipher)//block_size)]
    for i in range(1, len(Blocks)):
        if  Blocks[i]==Blocks[i-1]:
            preLength = (i+1)*block_size
            knowncipher = Blocks[i]
            break
    for i in range(block_size*2):
        tmp_cipher = encryption_oracle(b"A"*i)
        Blocks = [tmp_cipher[i*block_size:][:block_size] for i in range(len(tmp_cipher)//block_size)]
        if knowncipher in Blocks:
            preLength = Blocks.index(knowncipher)*block_size
            break
    inputPreFix = b"A"*(i-block_size)
    log("", f"Random Prefix Cipher Length: {preLength}")
    ...
    while(True):
        ...
        for bruteValue in range(256):
            key = encryption_oracle(inputPreFix + bruteInput+int2byte(bruteValue))[preLength:][:block_size]
            tables[key] = bruteValue
        # log("   ", f"Tables Generating Done.")
        inp = b"A"*((block_size-1-len(PlainText))%block_size)
        # log("   ", f"InputValue: {inp}")
        key = encryption_oracle(inputPreFix+inp)[preLength + (len(PlainText)//block_size)*block_size:][:16]
        # log("   ", f"UniqueKey: {key}")
        try:
