...
def setByte(array, offset, value):
    array = array[0:offset] + \
        value.to_bytes(1, byteorder='big') + array[offset+1:len(array)]
    return array
...
def setIV(rawIV, revIndex, x, knownPlaintext):
    ...
    extendedPlaintext = b'\x00' * \
        (len(rawIV) - len(knownPlaintext) - 1) + \
        x.to_bytes(1, byteorder='big') + knownPlaintext
    ...
    pad = b'\x00' * (len(rawIV) - revIndex - 1) + \
        ((revIndex+1).to_bytes(1, byteorder='big') * (revIndex+i))
    output = b''
    ...
    for i in range(len(rawBlock)):
        ...
        for x in range(256):
            ...
            if (checkPadding(rawBlock, thisIV)):
                ...
                if (i == 0):
                    thisIV = setByte(thisIV, len(thisIV)-2,
                                     thisIV[len(thisIV)-2] ^ 0xff)
                ...
                recoveredPlaintext = x.to_bytes(
                    1, byteorder='big') + recoveredPlaintext
